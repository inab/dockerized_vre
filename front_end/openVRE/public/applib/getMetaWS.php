...
?>
	<table class="table table-striped table-bordered">
		...
		<tr>
        	<td><?=printFilePath_fromPath($mt['path'], $asRoot);?></td>
    	</tr>
    	</tbody>
	</table>
<?php
...
// Data_type, file_type, and OTHER METADATA (for files)  CHECK OEB / MuG

if ($_REQUEST["type"] == 1) {
    // show data_type, file_type
    $dt = $GLOBALS['dataTypesCol']->findOne(['_id' => $mt["data_type"]]);

    // show taxon_id and assembly  TOCHECK OEB??
...
if (isset($mt['expiration'])) {
    if (is_object($mt['expiration'])){
        ...
        $mt['expiration'] = strftime('%Y/%m/%d %H:%M', $expiration_date); //TODO DEPRECATED function
        if ($days2expire < 0 ) {
...
<table class="table table-striped table-bordered">
	<tbody>
		...
		<tr>
		    <td>
		<?php if (count($mt['associated_files'])) { ?>
				<ul class="feeds" id="list-files-run-tools">
		<?php	foreach ($mt['associated_files'] as $inp) {
             	    $path = getAttr_fromGSFileId($inp, 'path', $asRoot);
                    print printFilePath_fromPath($path,$asRoot);
				} ?>
				</ul>
...
<table class="table table-striped table-bordered">
	<tbody>
        ...
        <?php   foreach ($mt["arguments"] as $k => $v) {
...
<table class="table table-striped table-bordered">
	<tbody>
		...
		<tr>
        	<td>
				<?php if ($mt['path']){echo $mt['path'];}else{echo "Job has no execution directory !!";}?>
            </td>
		</tr>
	</tbody>
</table>
...
<div id="meta-log">
	<?php
        ...
        if (file_exists($mt['log_file'])) { ?>
			<a href="workspace/workspace.php?op=openPlainFileFromPath&fnPath=<?=urlencode($mt['log_file'])?>" class="btn green" target="_blank"><i class="fa fa-file-text-o"></i> VIEW LOG FILE </a>
		<?php } else { ?>
        ...
        if (($_SESSION['User']['Type'] == 0) || ($_SESSION['User']['Type'] == 1)) { ?>
			...
			<?php if (file_exists($mt['submission_file'])) { ?>
				<a href="workspace/workspace.php?op=openPlainFileFromPath&fnPath=<?=urlencode($mt['submission_file'])?>" class="btn green" target="_blank"><i class="fa fa-paper-plane"></i> VIEW SUBMIT FILE </a>
			<?php } else { ?>
			...
			<?php }
			if (file_exists($mt['config_file'])) { ?>
				<a href="workspace/workspace.php?op=openPlainFileFromPath&fnPath=<?=urlencode($mt['config_file'])?>" class="btn green" target="_blank"><i class="fa fa-cog"></i> VIEW CONFIG FILE </a>
			...
			<?php if (file_exists($mt['metadata_file']) ) { ?>
				<a href="workspace/workspace.php?op=openPlainFileFromPath&fnPath=<?=urlencode($mt['metadata_file'])?>" class="btn green" target="_blank"><i class="fa fa-tags"></i> VIEW META FILE </a>
			<?php } elseif (file_exists($GLOBALS['dataDir'].$mt['path']."/".$GLOBALS['tool_metadata_file']) ) { ?>
				<a href="workspace/workspace.php?op=openPlainFileFromPath&fnPath=<?=urlencode($mt['path']."/".$GLOBALS['tool_metadata_file'])?>" class="btn green" target="_blank"><i class="fa fa-tags"></i> VIEW META FILE </a>
			<?php } else { ?>
			...
			<?php if (file_exists($mt['stageout_file']) ) { ?>
				<a href="workspace/workspace.php?op=openPlainFileFromPath&fnPath=<?=urlencode($mt['stageout_file']); ?>" class="btn green" target="_blank"><i class="fa fa-line-chart"></i> VIEW RESULTS FILE </a>
			<?php } elseif (file_exists($GLOBALS['dataDir'].$mt['path']."/".$GLOBALS['tool_stageout_file']) ) { ?>
				<a href="workspace/workspace.php?op=openPlainFileFromPath&fnPath=<?=urlencode($mt['path']."/".$GLOBALS['tool_stageout_file'])?>" class="btn green" target="_blank"><i class="fa fa-line-chart"></i> VIEW RESULTS FILE </a>
			<?php } else { ?>
