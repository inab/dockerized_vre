...
// TODO Reorganize to make index / statistics not mandatory
...
if ($_REQUEST) {
	...
	//
	if (!count($results)) {
		$files = $GLOBALS['filesCol']->findOne(['_id' => $_REQUEST['execution']], ['files' => 1, '_id' => 0]);

		$has_statistics = false;
		foreach ($files["files"] as $id) {

			$fMeta = iterator_to_array($GLOBALS['filesMetaCol']->find(
				['_id' => $id,'data_type'  => "tool_statistics", 'format' =>'TAR','compressed' =>"gzip"]
			));
